...
export class CandidateAddComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if( window.history.state.addCandidateEndpoint != undefined)
      localStorage.setItem('addCandidateEndpoint', window.history.state.addCandidateEndpoint);

      this.getInstitutionAndLocationData();
  }

  candidateForm: FormGroup = new FormGroup({
    name: new FormControl('', [Validators.required, Validators.minLength(3), Validators.maxLength(20), Validators.pattern("[a-zA-Z][a-zA-Z ]+")]),
  ...
  addCandidate(){
    if(this.candidateForm.valid){
      ...
      console.log(this.userId, this.selectedInstitutionId, this.selectedLocationId);
      ...
      this._service.postData(this.candidateData, this.addCandidateEndpoint).subscribe({
        next: response => {
