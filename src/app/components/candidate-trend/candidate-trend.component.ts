...
export class CandidateTrendComponent implements OnInit {
  ...
  preparePieChartData(){
    this.candidateData.forEach((candidate) => {
      ...
      this.locationMap.set(candidate.location.name, tempVal != undefined ? tempVal + 1 : 0);

      tempVal = this.institutionMap.get(candidate.institution.name);
      this.institutionMap.set(candidate.institution.name, tempVal != undefined ? tempVal + 1 : 0);
